/***************************************************************************\
* @file 23_Arduino_Lights_Neopixel.ino
* 
...
#define APPNAME "FRC102-LED-strip-2023-neopixel"

// Imports the Arduino NeoPixel library from Adafruit; needs to be installed through Arduino's library manager
// (library is called "Adafruit NeoPixel", to open the manager: Ctrl+Shift+I or "Tools" - > "Manage Libraries...")
//#include <Adafruit_DotStar.h>
#include <Adafruit_NeoPixel.h>
////////////////////// Strip configuration settings:
#define NUMPIXELS      60       // Number of LEDs in strip (usually 30, but we're using a shorter segment)
#define DATAPIN        4        // Pin for data input
//#define CLOCKPIN       11       // Pin for clock/timer input (not needed for neopixels)
...
#define SM_PREFIX      "[led_strip_2023]"        // Prefix for printing to the serial monitor
...
int readSerial() {
  if (Serial.available() > 0) {
    ...
    while (i < total) {
      ...
      else if (tolower(inp) == 's') {currentdir=!currentdir; return pattern;}
...
int reverseIndex(int i) {
  if (currentdir == true) return NUMPIXELS-i-1;
  else return i;
}
